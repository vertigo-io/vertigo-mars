...
public class MultipartConfigInjectionHandler extends HandlerWrapper {
	...
	static {
		final int maxFileSize = getIntFromParamOrEnv("MAX_FILE_SIZE").orElse(5); // default to 5 Mo, maxFileSize the maximum size allowed for uploaded files
		final int maxRequestSize = getIntFromParamOrEnv("MAX_REQUEST_SIZE").orElse(maxFileSize); // default to maxFileSize, maxRequestSize the maximum size allowed for multipart/form-data requests
		final int fileSizeThreshold = getIntFromParamOrEnv("FILE_SIZE_MEMORY_THRESHOLD").orElse(1); // default to 1 Mo fileSizeThreshold the size threshold after which files will be written to disk

		MULTI_PART_CONFIG = new MultipartConfigElement(
				System.getProperty("java.io.tmpdir"), //location the directory location where files will be stored
	...
	private static OptionalInt getIntFromParamOrEnv(final String paramName) {
		final String systemValue = System.getProperty(paramName);
		if (systemValue != null) {
			return OptionalInt.of(Integer.parseInt(systemValue));
		}
		final String envValue = System.getenv(paramName);
		if (envValue != null) {
			return OptionalInt.of(Integer.parseInt(envValue));
		}
